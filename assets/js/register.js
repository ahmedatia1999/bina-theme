// Register page behavior: validation, city loading, submission.
(function () {
  ...
  const els = {
    ...
    accountTypeCustomer: form.querySelector('input[type="radio"][value="customer"]'),
    accountTypeContractor: form.querySelector('input[type="radio"][value="contractor"]'),
    accountTypeValue: document.getElementById("accountTypeValue"),
    cityTrigger: document.getElementById("cityTrigger"),
  ...
  const messages = {
    firstName: msgEl("_R_29av5uivaiivb_-form-item-message"),
    email: msgEl("_R_dav5uivaiivb_-form-item-message"),
    phone: msgEl("_R_hav5uivaiivb_-form-item-message"),
    city: msgEl("_R_lav5uivaiivb_-form-item-message"),
    password: msgEl("_R_pav5uivaiivb_-form-item-message"),
    confirmPassword: msgEl("_R_tav5uivaiivb_-form-item-message"),
  };
  ...
  function getSiteBase() {
    ...
    const marker = "/wp-content/";
    const idx = path.indexOf(marker);
    if (idx !== -1) return `${origin}${path.slice(0, idx)}`;
    return origin;
  }
  ...
  function getAccountType() {
    ...
    const checkedInput =
      form.querySelector('input[type="radio"][name="accountType"]:checked') ||
      form.querySelector('input[type="radio"][value="customer"]:checked') ||
      form.querySelector('input[type="radio"][value="contractor"]:checked');
  ...
  function setupAccountTypeRadios() {
    ...
    labels.forEach((lab) => {
      lab.addEventListener("click", (e) => {
        // avoid double toggles if click lands on city dropdown buttons etc.
  ...
  function setupPasswordToggles() {
    ...
    wrappers.forEach((wrapper) => {
      ...
      // Move the icon away from the border a bit (RTL/LTR-safe because we use 'right' as in the markup).
      btn.style.right = "6px";
      btn.style.left = "auto";
  ...
  // Cities dropdown (simple, minimal)
  let cityDropdown;
  ...
  async function loadCities() {
    ...
    try {
      ...
      els.cityTrigger.disabled = false;
      // match original initial text after loading
      // add icon back
      if (!userSelectedCity) setCity("", "اختر المدينة");
      syncSelectedCityUI();
    } catch (err) {
      // Keep fallback list and let the user proceed.
      els.cityTrigger.disabled = false;
      if (!userSelectedCity) setCity("", "اختر المدينة");
      syncSelectedCityUI();
    }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    // Success: redirect to login like the original
    window.location.href = `${getSiteBase()}/login`;
  });
